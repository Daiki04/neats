...
class BaseCPPNDecoder:
    def feedforward(self, inputs, genome, config):
        cppn = FeedForwardNetwork.create(genome, config) # FeedForwardNetworkのインスタンスを作成

        states = [] # 各voxelの座標に対する出力を格納するリスト
        for inp in inputs:
            state = cppn.activate(inp)
            states.append(state)

        return np.vstack(states) # 各voxelの座標に対する出力を縦に並べて返す
