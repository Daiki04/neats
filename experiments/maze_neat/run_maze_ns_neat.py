...
def main():
    ...
    save_path = os.path.join(CURR_DIR, 'out', 'maze_elit_ns_neat', args.name)
    ...
    decode_function = elit_ns_neat.FeedForwardNetwork.create
    ...
    config = elit_ns_neat.make_config(config_file, custom_config=custom_config)
    config_out_file = os.path.join(save_path, 'maze_elit_ns_neat.cfg')
    config.save(config_out_file)


    pop = elit_ns_neat.Population(config)

    figure_path = os.path.join(save_path, 'progress')
    reporters = [
        elit_ns_neat.SaveResultReporter(save_path),
        elit_ns_neat.NoveltySearchReporter(True),
    ...
    # finally:
        # elit_ns_neat.figure.make_species(save_path)
