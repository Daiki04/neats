...
import neat_species_hyb_crossover_elit_tournament_repo as neat
...
def main():
    ...
    for trial in range(3, args.trials):
        # シード値を固定
        np.random.seed(trial)
        random.seed(trial)

        save_path = os.path.join(CURR_DIR, 'out', 'doublependulum_exp_repo_top', args.name+'_'+str(trial)) # 結果の保存先
