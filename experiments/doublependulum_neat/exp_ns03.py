...
def main():
    ...
    for trial in range(3, 4):
        ...
        save_path = os.path.join(
            CURR_DIR, 'out', 'doublependulum_exp', args.name+'_'+str(trial))  # 結果の保存先
        ...
        try:
            best_genome = pop.run(
                evaluate_function=parallel.evaluate, n=args.generation)

            print()
            print('best doublependulum result:')
            evaluator.print_result(decode_function(
                best_genome, config.genome_config))  # 最良個体の評価結果の表示
        finally:
            # neat.figure.make_species(save_path) # 種の進化の様子の保存
            evaluator.get_gif(decode_function(
                best_genome, config.genome_config), save_path)
            # pass
