    ...
    def reproduce(self, config, species, pop_size, generation, constraint_function=None):
        ...
        for s in remaining_species:
            ...
            while spawn > 0:
                ...
                if len(remaining_species) > 1 and random.random() < 0.1:
                    another_s = random.choice(remaining_species)
                    while another_s == s:
                        another_s = random.choice(remaining_species)
                    parent2_id, parent2 = random.choice(species_old_members[another_s.key])
                else:
