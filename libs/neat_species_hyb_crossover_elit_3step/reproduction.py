...
class DefaultReproduction(DefaultReproduction):
    ...
    def reproduce(self, config, species, pop_size, generation, constraint_function=None):
        ...
        for s in remaining_species:
            ...
            if spawn <= 0:
                continue
            stag_rate = (generation - s.last_improved) % self.stagnation.stagnation_config.max_stagnation
            stag_rate = stag_rate / \
            ...
            while spawn > 0:
                ...
                if stag_rate < 0.5 and species_af_runk_index[s.key] > 0 and len(remaining_species) > 0:
                    # step1: Cross over between species
                    tournament_size = 3 if len(old_members) > 10 else min(len(old_members), 2)
                    idx = list(range(len(old_members)))
                    tournament1 = random.sample(idx, tournament_size)
                    parent1_id, parent1 = old_members[min(tournament1)]
                ...
                elif species_af_runk_index[s.key] > 0 and stag_rate >= 0.5 and stag_rate < 0.7 and len(remaining_species) > 1:
                    # step2: Cross over with all members in the species
                    tournament_size = 3 if len(old_members) > 10 else min(len(old_members), 2)
                    idx = list(range(len(old_members)))
                    tournament1 = random.sample(idx, tournament_size)
                    parent1_id, parent1 = old_members[min(tournament1)]
                    tournament2 = random.sample(idx, tournament_size)
                    parent2_id, parent2 = old_members[min(tournament2)]
                elif species_af_runk_index[s.key] == 0 and stag_rate < 0.3:
                    # step2: Cross over with all members in the species
                    tournament_size = 3 if len(old_members) > 10 else min(len(old_members), 2)
                    idx = list(range(len(old_members)))
                    tournament1 = random.sample(idx, tournament_size)
                    ...
                    # step3: Cross over with elites in the species
                    repro_cutoff = int(math.ceil(self.reproduction_config.survival_threshold *len(old_members)))
                    repro_cutoff = max(repro_cutoff, 2)
