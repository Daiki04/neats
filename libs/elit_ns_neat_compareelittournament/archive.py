...
class NoveltyArchive(BaseArchive):
    ...
    def update_archive(self, population, elite_archive):
        ...
        for key, genome in archive_pop_and_novelty.items():
            ...
            max_sim_key = list(new_archive.keys())[np.argmax(cos_sim)]
            ratio_novelty = min(genome.novelty, new_archive[max_sim_key].novelty) / max(genome.novelty, new_archive[max_sim_key].novelty)
            if max_sim < 0.8 or ratio_novelty < 0.8:
                new_archive[key] = genome
                new_archive_data.append(genome.data)
...
class FitnessArchive(BaseArchive):
    ...
    def tournament_selection(self):
        k = 10 if len(self.archive) > 20 else 2
        selected = np.random.choice(list(self.archive.keys()), k, replace=False)
        selected = sorted(selected, key=lambda x: self.archive[x].fitness, reverse=True)
        id, genome = selected[0], self.archive[selected[0]]
        return id, genome
