        ...
        for s in remaining_species:
            ...
            while spawn > 0:
                ...
                if stag_rate < 0.5 and species_af_runk_index[s.key] > 0 and len(remaining_species) > 1:
                    tournament_size = 3 if len(
                        old_members) > 10 else min(len(old_members), 2)
                    ...
                    parent2_id, parent2 = species_old_members[another_s][min(
                        tournament2)]
                else:
                    tournament_size = 3 if len(
                        old_members) > 10 else min(len(old_members), 2)
                    idx1 = list(range(len(old_members)))
                    tournament1 = random.sample(idx1, tournament_size)
                    parent1_id, parent1 = old_members[min(tournament1)]
                    idx2 = list(range(len(old_members)))
                    tournament2 = random.sample(idx2, tournament_size)
