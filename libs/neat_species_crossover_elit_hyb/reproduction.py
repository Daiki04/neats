...
class DefaultReproduction(DefaultReproduction):
    ...
    def reproduce(self, config, species, pop_size, generation, constraint_function=None):
        ...
        species_id_af = [(s.key, s.adjusted_fitness) for s in remaining_species]
        species_id_af.sort(reverse=True, key=lambda x: x[1])
        species_af_runk = [sid for sid, _ in species_id_af]
        species_af_runk_index = {sid: idx for idx, sid in enumerate(species_af_runk)}
        ...
        for spawn, s in zip(spawn_amounts, remaining_species):
            ...
            if spawn <= 0:
                species_old_members[s.key] = old_members[:self.reproduction_config.elitism]
                continue
        ...
        for s in remaining_species:
            ...
            while spawn > 0:
                ...
                if stag_rate >= 0.5 and species_af_runk_index[s.key] > 0 and len(remaining_species) > 1:
                    # print('Stagnation rate:', stag_rate)
                    another_s = random.choice(species_af_runk[:species_af_runk_index[s.key]])
                    while another_s == s:
                        another_s = random.choice(species_af_runk[:species_af_runk_index[s.key]])
                    parent2_id, parent2 = random.choice(species_old_members[another_s])
                else:
