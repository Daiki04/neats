...
class FloatAttribute(BaseAttribute):
    ...
    def config_item_name(self, config_item_base_name: str) -> str:
        ...
        Returns:
            str: 属性名_設定名
        """
        name = self.name if "ctp" not in self.name else "ctp"
        return "{0}_{1}".format(name, config_item_base_name)
    ...
    def init_value(self, config):
        ...
        # num_points = getattr(config, self.num_points_name)

        # if num_points > 1:
        #     return [self.clamp(gauss(mean, stdev), config) for _ in range(num_points)]
        ...
        if 'ctp' in init_type:
            return gauss(mean, (stdev / getattr(config, self.num_points_name))**2)
...
class ListAttribute(BaseAttribute):
    """
    Class for list attributes such as the aggregation function of a node,
    which are selected from a list of options.
    """
    _config_items = {"num_points": [int, 1],
    ...
    def init_value(self, config, data_attr = FloatAttribute):
        num_points = getattr(config, self.num_points_name)
        data_arr = [data_attr(self.name + str(i)).init_value(config) for i in range(num_points)]
        return data_arr
    
    def mutate_value(self, value, config, data_attr = FloatAttribute):
        num_points = getattr(config, self.num_points_name)
        return [data_attr(self.name + str(i)).mutate_value(value[i], config) for i in range(num_points)]
