...
class Population:
    def __init__(self, config, initial_state=None, constraint_function=None):
        ...
        self.novelty_archive = archive.NoveltyArchive(config, pre_train=True)
    ...
    def run(self, evaluate_function, constraint_function=None, n=None):
        ...
        while n is None or k < n:
            ...
            self.novelty_archive.update_archive(self.population)
            ...
            self.population = self.reproduction.reproduce(
                self.config, self.fitness_archive, self.novelty_archive, self.config.pop_size, self.generation, novelty_flag)
            ...
            print(f'novelty size: {len(self.novelty_archive.archive)}, novelty threshold: {self.novelty_archive.novelty_threshold}, max: {max([g.novelty for g in self.novelty_archive.archive.values()])}')
            print(f'fitness size: {len(self.fitness_archive.archive)}, fitness max: {max([g.fitness for g in self.fitness_archive.archive.values()])}')
